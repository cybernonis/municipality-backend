...
def list_crews(
    ...
    db = get_db()
    query = db.table("crews").select("*, departments(id, name, slug)")
...
def get_crew(crew_id: str):
    ...
    result = db.table("crews").select("*, departments(id, name, slug)").eq(
        "id", crew_id
    ).execute()
    if not result.data:
...
async def create_crew(
    ...
    dept = db.table("departments").select("id").eq("id", department_id).execute()
    if not dept.data:
    ...
    result = db.table("crews").insert(row).execute()
...
async def update_crew(
    ...
    db = get_db()
    result = db.table("crews").update(update_data).eq("id", crew_id).execute()
...
async def delete_crew(
    ...
    """Διαγραφή συνεργείου (μόνο αν δεν έχει ενεργές αναφορές)."""
    if current_user.get("role") not in ["admin", "manager"]:
        raise HTTPException(status_code=403, detail="Δεν έχετε δικαίωμα")
...
def get_crew_reports(
    ...
    db = get_db()
    query = db.table("reports").select("*, departments(name)").eq("crew_id", crew_id)
